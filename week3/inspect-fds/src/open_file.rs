...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_file = fs::read_link(fd_path).ok()?;
        let name = Self::path_to_name(fd_file.to_str()?);
        
        let fdinfo_path = format!("/proc/{pid}/fdinfo/{fd}");
        let fdinfo = fs::read_to_string(fdinfo_path).ok()?;

        let cursor = Self::parse_cursor(fdinfo.as_str())?;
        let access_mode = Self::parse_access_mode(fdinfo.as_str())?;
